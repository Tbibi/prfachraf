...
export default function HeroButtons({ slide }: HeroButtonsProps) {
  ...
  const secondaryLabel = t(`slides.${slide.id}.secondaryLabel`);
  const primaryMessage = encodeURIComponent(
    t(`slides.${slide.id}.primaryMessage`)
  );
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
      ...
      >
        <a
          href={`https://wa.me/212600000000?text=${primaryMessage}`}
          target="_blank"
        ...
        >
          <span className="me-2 text-sm sm:me-2.5 sm:text-base">💬</span>
          {primaryLabel}
          <motion.span
